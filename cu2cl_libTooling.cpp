...
private:
	...
	else if (funcName == "clEnqueueReadBuffer")
	{
		...
		newExpr = "cudaMemcpy(" + newDst + ", " + newSrc + ", " + newCount + "," + "cudaMemcpyDeviceToHost)"; 
//TODO support offsets (will need to grab pointer out of cudaMemcpy
	}        // call, then separate off the rest of the math as the offset)
	...
	//	std::cout << newIndex << " " << typeid(newIndex).name() << std::endl;
	//	std::cout << newArg << std::endl;
		int idx = newIndex[0] - '0';
		a[idx] = newArg.substr(1);
	//	std::cout << a[idx] << std::endl;
	//	std::cout << "INSIDE KERNELARG" << std::endl;
		newExpr = "//";
	}
	...
	{
	//	std::cout << "INSIDE clCreateKernel" << std::endl;
	...
	//	std::cout << newKernelName << std::endl;
	//	std::cout << finalKerName << std::endl;		
	}
